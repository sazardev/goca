...
var configValidateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := validateConfiguration(); err != nil {
			// Exit non-zero so the command can be used as a CI gate.
			os.Exit(1)
		}
	},
}
...
// validateConfiguration validates the current .goca.yaml and returns an error
// when it is missing, malformed or structurally invalid. The caller decides the
// process exit code; this keeps the function testable.
...
func validateConfigStructure(config map[string]interface{}) []string {
	...
	if _, exists := config["project"]; !exists {
		errors = append(errors, "Required section 'project' missing")
	}

	// Database configuration may live under the legacy "defaults" section or the
	// modern "database" section generated by `goca init`. Accept either so the
	// tool's own generated configuration passes validation.
	if _, hasDefaults := config["defaults"]; !hasDefaults {
		if _, hasDatabase := config["database"]; !hasDatabase {
			errors = append(errors, "Required section 'defaults' (or 'database') missing")
		}
	}
