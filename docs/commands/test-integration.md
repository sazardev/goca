...
| File | Description |
| --- | --- |
| `internal/testing/integration/order_integration_test.go` | Full integration test suite |
...
Integration tests are supported for **3 databases**: `postgres`, `mysql`, and `sqlite`. Using `--container` spins up a real PostgreSQL or MySQL container via Testcontainers; without it, an in-memory SQLite database is used.
